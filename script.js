...
// Cache para resultados de aeroportos
const airportCache = {};
const API_KEY = '84765bd38cmsh03b2568c9aa4a0fp1867f6jsnd28a64117f8b';
const API_HOST = 'aerodatabox.p.rapidapi.com';

// Função para buscar coordenadas de aeroportos
async function getAirportCoordinates(query) {
    ...
    if (airports[upperQuery]) {
        airportCache[key] = airports[upperQuery];
    ...
    try {
        const url = `https://${API_HOST}/airports/search/term?q=${encodeURIComponent(query)}&limit=1`;
        ...
        if (data.items && data.items.length > 0) {
            ...
            const res = {
                coords: [item.location.lat, item.location.lon],
                name: item.municipalityName || item.name,
                icao: item.icao
            };
            airportCache[key] = res;
            return res;
        ...
        console.error('Erro ao consultar Aerodatabox:', err);
    }

    // Fallback fixo caso não encontre
    const fallback = { coords: [-15.7942, -47.8822], name: query };
    airportCache[key] = fallback;
    return fallback;
}
    ...
    document.getElementById('btCalc').addEventListener('click', async () => {
        try {
            ...
            for (let input of inputs) {
                ...
                const airportData = await getAirportCoordinates(val);
                places.push({
